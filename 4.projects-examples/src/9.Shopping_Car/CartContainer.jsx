...
const CarContainer = () => {
    const {dispatch,cartState,totalPrice} = useContextAPIGlobal();
  ...
  return (
    <>
      { (cartState.productList.size>0 && !cartState.loading) ?
          <>
            { Array.from(cartState.productList.keys()).map(id => <CartElement {...cartState.productList.get(id)} key={id}></CartElement>) }
            <hr></hr>
